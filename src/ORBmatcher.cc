...
int ORBmatcher::SearchByBoW(KeyFrame* pKF,Frame &F, vector<MapPoint*> &vpMapPointMatches)
{
    ...
    vpMapPointMatches = vector<MapPoint*>(F.N,static_cast<MapPoint*>(NULL)); /** @attention MapPoints的数目与Frame中的关键点数目相同*/
    ...
    /*【步骤0】: 将属于同一节点(特定层)的ORB特征进行匹配*/
    // 参考: SearchForTriangulation 函数，注释更详细
...
int ORBmatcher::SearchByProjection(KeyFrame* pKF, cv::Mat Scw, const vector<MapPoint*> &vpPoints, vector<MapPoint*> &vpMatched, int th)
{
    ...
    const float scw = sqrt(sRcw.row(0).dot(sRcw.row(0)));/** @done 计算得到尺度s，见公式推导*/
    cv::Mat Rcw = sRcw/scw;
...
int ORBmatcher::SearchForInitialization(Frame &F1, Frame &F2, vector<cv::Point2f> &vbPrevMatched, vector<int> &vnMatches12, int windowSize)
{
    ...
    for(size_t i1=0, iend1=F1.mvKeysUn.size(); i1<iend1; i1++)
    {
        ...
        for(vector<size_t>::iterator vit=vIndices2.begin(); vit!=vIndices2.end(); vit++)
        {
            ...
            if(vMatchedDistance[i2]<=dist) //再次匹配到之前已经匹配上的特征点时，需做此处理，选出最佳匹配
                continue;
...
/**
 * @brief 通过词包，对关键帧的特征点进行跟踪，该函数用于<闭环检测>时两个关键帧间的特征点匹配
 *
 * 通过bow对pKF和F中的特征点进行快速匹配（不属于同一node的特征点直接跳过匹配） \n
 * 对属于同一node的特征点通过描述子距离进行匹配 \n
 * 根据匹配，更新vpMatches12 \n
 * 通过距离阈值、比例阈值和角度投票进行剔除误匹配
 * @param  pKF1               mpCurrentKF
 * @param  pKF2               闭环候选帧
                                       ...
                                       vector<pair<size_t, size_t> > &vMatchedPairs, const bool bOnlyStereo)
{
    ...
    // f1it->first对应node编号(图像金字塔的层)，f1it->second对应属于该node的所有<特征点编号>
    DBoW2::FeatureVector::const_iterator f1it = vFeatVec1.begin();
    DBoW2::FeatureVector::const_iterator f2it = vFeatVec2.begin();
    DBoW2::FeatureVector::const_iterator f1end = vFeatVec1.end();
    DBoW2::FeatureVector::const_iterator f2end = vFeatVec2.end();
    ...
    while(f1it!=f1end && f2it!=f2end)
    {
    /** @attention 如果f1it和f2it属于同一个node节点(同一图像金字塔层)*/
        if(f1it->first == f2it->first)
        {
    /*【步骤2】： 遍历该node节点下(f1it->first)的所有特征点*/
            for(size_t i1=0, iend1=f1it->second.size(); i1<iend1; i1++)
            {
                // 获取pKF1中属于该<node节点>的<特征点索引>
                const size_t idx1 = f1it->second[i1];
    ...
    /*【步骤2.2】： 通过<特征点索引idx1>，在pKF1中取出对应的特征点*/
    ...
    /*【步骤3】： 遍历该node节点下(f2it->first)的所有特征点*/
                for(size_t i2=0, iend2=f2it->second.size(); i2<iend2; i2++)
                {
                    // 获取pKF2中属于该node节点的<特征点索引>
                    size_t idx2 = f2it->second[i2];

    /*【步骤3.1】： 通过特征点索引idx2，在pKF2中取出对应的MapPoint*/
    ...
    /*【步骤3.2】： 通过特征点索引idx2在pKF2中取出对应的特征点的描述子*/
                    ...
                    // 计算idx1与idx2在两个关键帧中对应特征点的描述子距离(汉明距离)
                    const int dist = DescriptorDistance(d1,d2);

    /** @attention 汉明距离=>>{在当前尺度下(理论上pKF1与pKF2尺度(金字塔层)相同)}=>>匹配成功*/
    ...
    /** @attention 阈值要根据金字塔层调整，层(octave)越高，尺度越大，阈值越大*/
    ...
    /*【步骤4】： 验证是否满足对极约束*/
                    ...
                    if(CheckDistEpipolarLine(kp1,kp2,F12,pKF2)) //kp1来自mpCurrentKeyFrame，kp2来自共视图
                    {
                        bestIdx2 = idx2;
                        bestDist = dist;
                    }
                ...
                // 步骤1、2、3、4总结下来就是：将左图像的每个特征点与右图像<同一node节点>的所有特征点
                // 依次检测，判断是否满足对极几何约束，满足约束就是匹配的特征点

                // 详见SearchByBoW(KeyFrame* pKF,Frame &F, vector<MapPoint*> &vpMapPointMatches)函数步骤4
                if(bestIdx2>=0) // bestIdx2初始值为-1，bestIdx2<0表示未找到匹配点
                {
                    const cv::KeyPoint &kp2 = pKF2->mvKeysUn[bestIdx2];
                    vMatches12[idx1]=bestIdx2; //两匹配关键点<索引>对应关系
                    vbMatched2[bestIdx2]=true;
                    nmatches++;
    /*【步骤5】: 匹配点方向差一致性约束*/
                    // 将关键帧与当前帧匹配点的angle相减，得到rot（0<=rot<360），放入一个直方图中，
                    // 对于每一对匹配点的角度差，均可以放入一个bin的范围内（360/HISTO_LENGTH）。
                    if(mbCheckOrientation)
                    {
                        float rot = kp1.angle-kp2.angle; //kp1.angle在ORBextractor.cc中的computeOrientation函数计算
                        if(rot<0.0)
                            rot+=360.0f;
                        int bin = round(rot*factor); //factor = HISTO_LENGTH/360.0f
                        if(bin==HISTO_LENGTH)
                            bin=0;
                        assert(bin>=0 && bin<HISTO_LENGTH);
                        rotHist[bin].push_back(idx1); /** @attention 统计直方图 */
                    }
                }
            }
        ...
        {
            //lower_bound函数: Finds the beginning of a subsequence matching given key.
            f1it = vFeatVec1.lower_bound(f2it->first);
        }
        else
        {
    ...
    if(mbCheckOrientation)
    {
        ...
        // 若最高的比第三高的高10倍以上，则保留最高的和第二高bin中的匹配点。
        ComputeThreeMaxima(rotHist,HISTO_LENGTH,ind1,ind2,ind3);
    ...
    vMatchedPairs.clear();
    vMatchedPairs.reserve(nmatches); //根据nmatches设置vector大小
...
int ORBmatcher::Fuse(KeyFrame *pKF, const vector<MapPoint *> &vpMapPoints, const float th)
{
    ...
    /*【步骤3】： PO与Pn的夹角须小于60度*/
        ...
        // Search in a radius
    /*【步骤4】： 根据MapPoint的深度确定尺度，从而确定搜索范围*/
        /** @attention 尺度越大，层越高，提取出该关键点时图像的分辨率越小，因此需要在更大的范围内搜索特征点*/
        const float radius = th*pKF->mvScaleFactors[nPredictedLevel];

    /*【步骤4.1】： 获取半径范围内关键点,加速匹配*/
        const vector<size_t> vIndices = pKF->GetFeaturesInArea(u,v,radius);
    ...
    /*【步骤5.1】: 计算MapPoint投影的坐标与这个区域特征点的距离，如果偏差很大，直接跳过特征点匹配*/
    ...
    /*【步骤5.1.1】: 基于卡方检验计算出的阈值（假设测量有一个像素的偏差）*/
                if(e2*pKF->mvInvLevelSigma2[kpLevel] > 5.99) /** @todo */
                    continue;
            }
...
int ORBmatcher::Fuse(KeyFrame *pKF, cv::Mat Scw, const vector<MapPoint *> &vpPoints, float th, vector<MapPoint *> &vpReplacePoint)
{
    ...
    for(int iMP=0; iMP<nPoints; iMP++)
    {
        ...
        for(vector<size_t>::const_iterator vit=vIndices.begin(); vit!=vIndices.end(); vit++)
        {
            ...
            int dist = DescriptorDistance(dMP,dKF); //比较描述子
        ...
        if(bestDist<=TH_LOW)
        {
            ...
            {
                if(!pMPinKF->isBad())
                    vpReplacePoint[iMP] = pMPinKF;/** @attention 记录下来该pMPinKF需要被替换掉*/
            }
                             ...
                             const float &s12, const cv::Mat &R12, const cv::Mat &t12, const float th)
{
    ...
    for(int i1=0; i1<N1; i1++)
    {
        ...
        const float maxDistance = pMP->GetMaxDistanceInvariance();
        const float minDistance = pMP->GetMinDistanceInvariance();
        const float dist3D = cv::norm(p3Dc2); /** @attention 相机中心到3D点的距离，比深度(Z轴坐标)更大*/
        ...
        // Search in a radius
        // 计算特征点搜索半径
        const float radius = th*pKF2->mvScaleFactors[nPredictedLevel]; //th=7.5
        ...
        for(vector<size_t>::const_iterator vit=vIndices.begin(), vend=vIndices.end(); vit!=vend; vit++)
        {
            ...
            if(kp.octave<nPredictedLevel-1 || kp.octave>nPredictedLevel) //不能跟预测的层差距太大
                continue;
...
int ORBmatcher::SearchByProjection(Frame &CurrentFrame, const Frame &LastFrame, const float th, const bool bMono)
{
    ...
    const bool bBackward = (-tlc.at<float>(2) > CurrentFrame.mb) && !bMono; /** @attention */
