...
void ImageGrabber::PublishPose(cv::Mat Tcw)
{
    ...
    if(!Tcw.empty())
    {
        ...
        cv::Mat Rwc = Tcw.rowRange(0,3).colRange(0,3).t(); //转置即是求逆
        cv::Mat twc = -Rwc*Tcw.rowRange(0,3).col(3); //相当于T_c_w_.inverse().translation()
        ...
        */
        ...
        poseMSG.pose.position.y = twc.at<float>(2); //相机坐标系与我们习惯的平面坐标系不一样，调换y与z即可
...
int main(int argc, char **argv)
{
    ...
    ORB_SLAM2::System SLAM(argv[1],argv[2],ORB_SLAM2::System::MONOCULAR,true, bReuseMap); //初始化
