...
void KeyFrame::UpdateConnections()
{
    ...
    /*【步骤1】: 通过3D点间接统计可以观测到这些3D点的所有关键帧之间的共视程度*/
    ...
    /*【步骤2】: vPairs记录与其它关键帧共视帧数大于th的关键帧*/
    ...
    for(map<KeyFrame*,int>::iterator mit=KFcounter.begin(), mend=KFcounter.end(); mit!=mend; mit++)
    {
        ...
        if(mit->second>=th) //th=15
        {
            // 对应权重需要大于阈值，对这些关键帧建立连接
            vPairs.push_back(make_pair(mit->second,mit->first));
            // 更新KFcounter中该关键帧的mConnectedKeyFrameWeights
    /*【步骤2.1】: 更新其它KeyFrame的mConnectedKeyFrameWeights，更新其它关键帧与当前帧的连接权重*/
            (mit->first)->AddConnection(this,mit->second);
        }
    }

    /*【步骤2.2】: 如果没有超过阈值的权重，则对权重最大的关键帧建立连接*/
    if(vPairs.empty())
    {
    ...
    /*【步骤3】: vPairs里存的都是相互共视程度比较高的关键帧和共视权重，由大到小*/
    sort(vPairs.begin(),vPairs.end());
    list<KeyFrame*> lKFs;
    list<int> lWs;
    ...
    /*【步骤4.2】: 更新生成树的连接*/
        ...
        {
            /** @attention 初始化该关键帧的父关键帧为共视程度最高的那个关键帧*/
...
{
    {
        ...
        {
            mbNotErase = false; //再调用SetBadFlag函数，即可删除关键帧
        }
    }

    /** @todo 这个地方是不是应该：(!mbToBeErased)，(wubo???)*/
...
vector<size_t> KeyFrame::GetFeaturesInArea(const float &x, const float &y, const float &r) const
{
    ...
    // floor向下取整，mfGridElementWidthInv为每个像素占多少个格子,对!
    // mfGridElementWidthInv = 64/(mnMaxX-mnMinX)
    ...
    for(int ix = nMinCellX; ix<=nMaxCellX; ix++)
    {
        ...
        {
            const vector<size_t> vCell = mGrid[ix][iy]; //获取该格子里面存储的关键点
            for(size_t j=0, jend=vCell.size(); j<jend; j++)
            {
                ...
                if(fabs(distx)<r && fabs(disty)<r) //在半径范围内，压栈
