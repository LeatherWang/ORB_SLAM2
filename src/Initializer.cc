                             ...
                             vector<cv::Point3f> &vP3D, vector<bool> &vbTriangulated)
{
    ...
    /*【步骤2】: 在所有匹配特征点对中<随机>选择8对匹配特征点为一组，共选择mMaxIterations组*/
    ...
    for(int it=0; it<mMaxIterations; it++)
    {
        ...
        for(size_t j=0; j<8; j++)
        {
            ...
            // randi对应的索引用<最后一个元素>替换，并删掉最后一个元素
...
void Initializer::FindFundamental(vector<bool> &vbMatchesInliers, float &score, cv::Mat &F21)
{
    ...
    vector<cv::Point2f> vPn1, vPn2;
    cv::Mat T1, T2;
    Normalize(mvKeys1,vPn1, T1); // 归一化<特征点>到同一尺度
    Normalize(mvKeys2,vPn2, T2); // 归一化<特征点>到同一尺度
    cv::Mat T2t = T2.t();
    ...
    for(int it=0; it<mMaxIterations; it++)
    {
        ...
        // 利用重投影误差为当次RANSAC的结果评分
        currentScore = CheckFundamental(F21i, vbCurrentInliers, mSigma); /** @todo*/
...
float Initializer::CheckFundamental(const cv::Mat &F21, vector<bool> &vbMatchesInliers, float sigma)
{
    ...
    for(int i=0; i<N; i++)
    {
        ...
        // x2应该在l这条线上:x2点乘l = 0，(这是F矩阵的推导出来的性质)
...
void Initializer::Triangulate(const cv::KeyPoint &kp1, const cv::KeyPoint &kp2, const cv::Mat &P1, const cv::Mat &P2, cv::Mat &x3D)
{
    ...
    x3D = vt.row(3).t(); // 约束: ||vt.row(3).t()||=1
    x3D = x3D.rowRange(0,3)/x3D.at<float>(3); //转换为齐次性坐标
}

/**
 * ＠brief 归一化<特征点>到同一尺度（作为normalize DLT的输入）
 *
 * [x' y' 1]' = T * [x y 1]' \n
...
void Initializer::Normalize(const vector<cv::KeyPoint> &vKeys, vector<cv::Point2f> &vNormalizedPoints, cv::Mat &T)
{
    ...
    // 将x坐标和y坐标分别进行尺度缩放，使得x坐标和y坐标的<一阶绝对矩>分别为1，<一阶矩>为0
    for(int i=0; i<N; i++)
    {
