...
{
    mpKF1 = pKF1; //mpCurrentKF
    mpKF2 = pKF2; //mvpEnoughConsistentCandidates[i]
    ...
    for(int i1=0; i1<mN1; i1++)
    {
        ...
        if(vpMatched12[i1])
        {
            /** @attention pMP1和pMP2是匹配的MapPoint*/
    ...
    SetRansacParameters(); /** @attention 使用函数初始化一组默认参数，用户亦可手动调用该函数修改参数*/
}

void Sim3Solver::SetRansacParameters(double probability, int minInliers, int maxIterations)//0.99,20,300
{
    ...
    if(mRansacMinInliers==N)
        ...
        nIterations = ceil(log(1-mRansacProb)/log(1-pow(epsilon,3))); /** @todo */

    mRansacMaxIts = max(1,min(nIterations,mRansacMaxIts)); //总的最大Ransac迭代次数
...
void Sim3Solver::CheckInliers()
{
    ...
    for(size_t i=0; i<mvP1im1.size(); i++)
    {
        ...
        if(err1<mvnMaxError1[i] && err2<mvnMaxError2[i]) //阈值跟尺度有关，尺度越大，阈值越高
        {
            mvbInliersi[i]=true;
            mnInliersi++;
        }
