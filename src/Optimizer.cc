                                 ...
                                 int nIterations, bool* pbStopFlag, const unsigned long nLoopKF, const bool bRobust)
{
    ...
    for(size_t i=0; i<vpMP.size(); i++)
    {
        ...
        vPoint->setId(id);
        vPoint->setMarginalized(true); //消元
    ...
    /*【步骤3】：向优化器添加投影边*/
        for(map<KeyFrame*,size_t>::const_iterator mit=observations.begin(); mit!=observations.end(); mit++)
        {
            ...
            if(pKF->mvuRight[mit->second]<0)
            {
                ...
                e->setInformation(Eigen::Matrix2d::Identity()*invSigma2); /** @attention 信息矩阵跟尺度的关系?*/
...
void Optimizer::LocalBundleAdjustment(KeyFrame *pKF, bool* pbStopFlag, Map* pMap)
{
    ...
    for(list<KeyFrame*>::iterator lit=lLocalKeyFrames.begin(), lend=lLocalKeyFrames.end(); lit!=lend; lit++)
    {
        ...
        vSE3->setFixed(pKFi->mnId==0);/** @attention 第一帧位置固定*/
        optimizer.addVertex(vSE3);
        ...
        for(map<KeyFrame*,size_t>::const_iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            ...
            if(!pKFi->isBad())
            {
                ...
                if(pKFi->mvuRight[mit->second]<0)
                {
                    ...
                    e->setInformation(Eigen::Matrix2d::Identity()*invSigma2); /** @todo */
    ...
    /*【步骤10】： 检测outlier，并设置下次不优化*/
        for(size_t i=0, iend=vpEdgesMono.size(); i<iend;i++)
        {
            ...
            // 基于卡方检验计算出的阈值（假设测量有一个像素的偏差）
            if(e->chi2()>5.991 || !e->isDepthPositive()) /** @todo */
            {
                e->setLevel(1);// 不优化
            }
                                       ...
                                       const map<KeyFrame *, set<KeyFrame *> > &LoopConnections, const bool &bFixScale)
{
    ...
    for(map<KeyFrame *, set<KeyFrame *> >::const_iterator mit = LoopConnections.begin(), mend=LoopConnections.end(); mit!=mend; mit++)
    {
        ...
        for(set<KeyFrame*>::const_iterator sit=spConnections.begin(), send=spConnections.end(); sit!=send; sit++)
        {
            ...
            /** @attention 权重不能低于100,EssentialGrahp*/
    ...
    /*【步骤4.2】：添加在CorrectLoop函数中AddLoopEdge函数添加的闭环连接边（当前帧与闭环匹配帧之间的连接关系）*/
        ...
        /*【步骤4.3】：对有很好共视关系的关键帧也作为边进行优化*/
        // 使用经过Sim3调整前关键帧之间的相对关系作为边
        const vector<KeyFrame*> vpConnectedKFs = pKF->GetCovisiblesByWeight(minFeat);
        for(vector<KeyFrame*>::const_iterator vit=vpConnectedKFs.begin(); vit!=vpConnectedKFs.end(); vit++)
        {
            KeyFrame* pKFn = *vit;
            //排除步骤4.1、步骤4.2,防止重复添加
    ...
    for(size_t i=0, iend=vpMPs.size(); i<iend; i++)
    {
        ...
        // 得到MapPoint参考关键帧，步骤5<优化前>的位姿
        g2o::Sim3 Srw = vScw[nIDr];
        // 得到MapPoint参考关键帧，步骤5<优化后>的位姿
...
int Optimizer::OptimizeSim3(KeyFrame *pKF1, KeyFrame *pKF2, vector<MapPoint *> &vpMatches1, g2o::Sim3 &g2oS12, const float th2, const bool bFixScale)
{
    ...
    /*【步骤2.2】: 添加PointXYZ顶点*/
                ...
                vPoint1->setId(id1);
                vPoint1->setFixed(true); /** @attention 不优化?*/
    ...
    /*【步骤2.3】: 添加两个顶点（3D点）到相机投影的边*/
        ...
        g2o::RobustKernelHuber* rk1 = new g2o::RobustKernelHuber; /** @todo */
        ...
        g2o::EdgeInverseSim3ProjectXYZ* e21 = new g2o::EdgeInverseSim3ProjectXYZ(); //逆Sim3
