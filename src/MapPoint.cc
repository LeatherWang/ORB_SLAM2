...
/**
 * @brief 1、擦出MapPoint的观测帧
 *        2、当能观测到该MapPoint的关键帧数目少于等于2时，删除该MapPoint
 * @param pKF
 */
void MapPoint::EraseObservation(KeyFrame* pKF)
{
    bool bBad=false;
    {
        ...
        if(mObservations.count(pKF))
        {
    /*【步骤1】: 擦出一个关键帧，则减少一个<能观测到><MapPoint>的<关键帧数目(nObs)>*/
            int idx = mObservations[pKF];
            if(pKF->mvuRight[idx]>=0)
                nObs-=2; //双目或者RGBD，观测数目需要减掉两张图片
            else
                nObs--; //单目，减到一张图片

    /*【步骤2】: 擦出MapPoint的观测关键帧*/
    ...
    /*【步骤3】: 如果少于2个关键帧观测到该MapPoint，则删除该MapPoint*/
            if(nObs<=2)
                bBad=true;
        }
    }

    // 少于2个关键帧观测到该MapPoint，则删除该MapPoint
    if(bBad)
        SetBadFlag();
}
...
void MapPoint::SetBadFlag()
{
    map<KeyFrame*,size_t> obs;
    {
        ...
        mbBad=true; //【1】: mbBad设置为true
        obs = mObservations;// 把mObservations转存到obs，obs和mObservations里存的是指针，赋值过程为浅拷贝
        mObservations.clear();//【2】: 把mObservations指向的内存释放，<obs作为局部变量之后自动删除>
    }

    // 【3】: 在所有能观测到该MapPoint的关键帧中，擦出该MapPoint
...
void MapPoint::ComputeDistinctiveDescriptors()
{
    ...
    /*【步骤1】: 遍历观测到3d点的所有关键帧，获得orb描述子，并插入到vDescriptors中*/
...
void MapPoint::UpdateNormalAndDepth()
{
    ...
    for(map<KeyFrame*,size_t>::iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
    {
        ...
        normal = normal + normali/cv::norm(normali); // 对<所有关键帧>对该点的观测方向<归一化为单位向量>进行求和
        n++;
    }
    ...
    // 通常来说，距离较近的地图点，将在金字塔层数较高的地方提取出，距离较远的地图点，在金字塔层数较低的地方提取出
    // （金字塔层数越低，分辨率越高，才能识别出远点）。
    {
...
int MapPoint::PredictScale(const float &currentDist, KeyFrame* pKF)
{
    ...
    // 同时取log线性化
    int nScale = ceil(log(ratio)/pKF->mfLogScaleFactor); //得到层
    if(nScale<0)
        nScale = 0;
