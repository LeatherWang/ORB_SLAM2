...
void KeyFrame::SetPose(const cv::Mat &Tcw_)
{
    ...
    cv::Mat Rwc = Rcw.t();
    Ow = -Rwc*tcw; //相机中心坐标，见笔记公式推导

    Twc = cv::Mat::eye(4,4,Tcw.type()); //生成单位矩阵时，需要设置元素数值类型
    Rwc.copyTo(Twc.rowRange(0,3).colRange(0,3));
    Ow.copyTo(Twc.rowRange(0,3).col(3));
    cv::Mat center = (cv::Mat_<float>(4,1) << mHalfBaseline, 0 , 0, 1);
    Cw = Twc*center; //立体相机中心坐标
}
...
/**
 * @brief KeyFrame::EraseMapPointMatch
 * @param idx
 * @note 在MapPoint.cc 中SetBadFlag函数调用
 */
    ...
    for(int i=0; i<N; i++)
    {
        ...
        if(pMP)
        {
            if(!pMP->isBad())
            {
                if(bCheckObs)
                {
                    if(mvpMapPoints[i]->Observations()>=minObs) //该MapPoint被观测到的次数大于minObs
                        nPoints++;
                }
                else
                    nPoints++;
