...
vector<KeyFrame*> KeyFrameDatabase::DetectLoopCandidates(KeyFrame* pKF, float minScore)
{
    ...
    /*【步骤1.1】： words是检测图像是否匹配的枢纽，遍历该pKF的<每一个word>*/
        for(DBoW2::BowVector::const_iterator vit=pKF->mBowVec.begin(), vend=pKF->mBowVec.end(); vit != vend; vit++)
        {
    /*【步骤1.2】：提取所有包含该word的KeyFrame*/
            list<KeyFrame*> &lKFs =   mvInvertedFile[vit->first];

    /*【步骤1.3】： 遍历包含该word的所有关键帧*/
            for(list<KeyFrame*>::iterator lit=lKFs.begin(), lend= lKFs.end(); lit!=lend; lit++)
            {
                KeyFrame* pKFi=*lit;
                if(pKFi->mnLoopQuery!=pKF->mnId)// pKFi还没有标记为pKF的候选帧
                {
                    ...
                    {
    /*【步骤1.4】： 将pKFi标记为pKF的候选帧，之后直接跳过判断*/
                        pKFi->mnLoopQuery=pKF->mnId;
                        lKFsSharingWords.push_back(pKFi);
                    }
                }

    /*【步骤1.5】：记录pKFi与pKF具有相同word的个数*/
                pKFi->mnLoopWords++;
            }
        }
    }
    ...
    /*【步骤2】：统计所有闭环候选帧中与pKF具有<共同单词最多的>单词数*/
    ...
    /*【步骤3】：遍历所有闭环候选帧，挑选出共有<单词数大于minCommonWords>且、<单词匹配度大于minScore>存入lScoreAndMatch*/
    for(list<KeyFrame*>::iterator lit=lKFsSharingWords.begin(), lend= lKFsSharingWords.end(); lit!=lend; lit++)
    {
        KeyFrame* pKFi = *lit;

    /*【步骤3.1】：pKF只和具有共同单词较多的关键帧进行比较，需要大于minCommonWords*/
        if(pKFi->mnLoopWords > minCommonWords)
        {
            nscores++;// 这个变量后面没有用到

            float si = mpVoc->score(pKF->mBowVec,pKFi->mBowVec); /** @attention 计算得分*/

            pKFi->mLoopScore = si;
    /*【步骤3.2】： 单词匹配度大于minScore*/
            if(si>=minScore)
    ...
    /*【步骤4.1】： 遍历每一组,计算组得分并记录该组分数最高的KeyFrame*/
    ...
    /*【步骤5】：得到组得分大于minScoreToRetain的组，得到<组中分数最高的关键帧>*/
...
vector<KeyFrame*> KeyFrameDatabase::DetectRelocalizationCandidates(Frame *F)
{
    ...
    for(list<pair<float,KeyFrame*> >::iterator it=lScoreAndMatch.begin(), itend=lScoreAndMatch.end(); it!=itend; it++)
    {
        ...
        {
            KeyFrame* pKF2 = *vit;
            if(pKF2->mnRelocQuery!=F->mnId) /** @attention 只有pKF2也在闭环候选帧中，才能贡献分数*/
                continue;
