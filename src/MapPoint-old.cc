...
void MapPoint::AddObservation(KeyFrame* pKF, size_t idx)
{
    ...
    if(mObservations.count(pKF))
        return;
    mObservations[pKF]=idx; //向map中插入一对数据,关键帧+该MapPoint在该帧中的编号
...
void MapPoint::EraseObservation(KeyFrame* pKF)
{
    ...
    /*【步骤2】: 擦出MapPoint的观测关键帧*/
            ...
            if(mpRefKF==pKF) //mpRefKF在MapPoint构造时使用pKFcur赋值
...
void MapPoint::SetBadFlag()
{
...

    {
        ...
        obs = mObservations;
        mObservations.clear(); //【2】: 清除所有的观测信息，释放内存
    }
    ...
    for(map<KeyFrame*,size_t>::iterator mit=obs.begin(), mend=obs.end(); mit!=mend; mit++)
    {
        ...
        pKF->EraseMapPointMatch(mit->second); //通过该MapPoint在该keyFrame中的索引，擦出之
    }
